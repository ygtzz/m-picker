class Picker{
    constructor(opts){
        ...
        this.wheels.forEach((item,index) => {
            ...
            item.addEventListener('touchstart',self.touchstart.bind(self,item,index));
            item.addEventListener('touchmove',self.touchmove.bind(self));
            item.addEventListener('touchend',self.touchend.bind(self,item,index));
    
            this.movec = item.querySelector('.pickers');
            this.setActiveItem(this.defaultIndex[index]);
        });
    }
    build(){
    ...
    touchend(e){
        ...
        if(touchEnd - this.touchStart < 300){
            console.log('快滑');
            this.translateTime(this.movec, this.detlaY * interiaRatio, 700, function(){
                self.fixScroll();
            });
        }
        //慢滑
    ...
    fixScroll(activeIndex){
        var currentY = this.getTranslate(this.movec, 'y');
        ...
        if(currentY > bottomBoundary){
            this.translateTime(this.movec, bottomBoundary - currentY, transTime);
            activeIndex = 0;
        }
        else if(currentY < topBoundary){
            this.translateTime(this.movec, topBoundary - currentY, transTime);
            ...
            for(var i=0;i<this.itemLen;i++){
                ...
                if(top < currentY && currentY <= bottom){
                    if(Math.abs(top - currentY) < Math.abs(currentY - bottom)){
                        this.translateTime(this.movec, top - currentY, transTime);
                        activeIndex = i+1;
                    }
                    else{
                        this.translateTime(this.movec, bottom - currentY, transTime);
                        activeIndex = i;
                    }
                }
            }
    ...
    setActiveItem(activeIndex){
        ...
        this.translateTime(this.movec, this.height/2 - this.itemHeight - activeIndex * this.itemHeight);
        this.changeActiveItem(activeIndex);
    }
    move(dom,detlaY){
        let originY = this.getTranslate(dom,'y');
        let y = originY + detlaY;
    
        this.translate(dom, y);
    }
    translate(dom,y){
        dom.style.webkitTransform = dom.style.transform = 'translate3d(0,'+y+'px,0)';
    }
    getTranslate(dom,type){
